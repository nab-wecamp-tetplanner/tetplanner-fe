...
const TaskColumn: React.FC<TaskColumnProps> = ({
  ...
  const [dissolvingTaskId, setDissolvingTaskId] = React.useState<string | null>(
    null,
  );
  ...
  const getStatusAccent = () => {
    switch (status) {
      case "pending":
        return "tet-col--amber";
      case "in_progress":
        return "tet-col--rose";
      case "completed":
        return "tet-col--emerald";
      case "cancelled":
        return "tet-col--slate";
      default:
        return "tet-col--amber";
    }
  };
        ...
        {tasks.length > 0 ? (
          tasks.map((task) => (
            <TaskCard
              ...
              onClick={(t) => {
                console.log(
                  "🟡 TaskColumn.onClick called for task:",
                  t.id,
                  "onTaskClick defined?",
                  !!onTaskClick,
                );
          ...
          <div
            className={`tet-col__empty ${isOver ? "tet-col__empty--active" : ""}`}
          >
            <span>Drop here</span>
          </div>
        )}
