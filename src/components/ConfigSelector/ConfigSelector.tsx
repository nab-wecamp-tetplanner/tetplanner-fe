...
import { Loader2, Plus, Check, Folder, Calendar, DollarSign } from "lucide-react";
import { useAppStore } from "../../stores/useAppStore";
import { toast } from "react-toastify";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import apiClient from "../../services/apiClient";
import type { TetConfig } from "../../types/tetConfig.types"; // Importing your actual type
...
const ConfigSelector: React.FC<ConfigSelectionProps> = ({ configs, isLoading }) => {
  ...
  const createMutation = useMutation({
    mutationFn: (newConfigData: { name: string; year: number; total_budget: number }) => {
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      name: name,
      year: Number(year),
      total_budget: totalBudget === "" ? 0 : Number(totalBudget),
    });
  };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center bg-background px-4">
      <div className="max-w-md w-full bg-card rounded-2xl border border-border p-8 shadow-sm">
        <h2 className="text-xl font-bold text-center mb-6">Welcome! Please set up your workspace</h2>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-600 mb-2">Select an existing workspace:</p>
            {configs.map((conf) => (
               <button
                 key={(conf as any).id}
               ...
               >
                 <div>
                   ...
                   {conf.year && <span className="text-xs text-gray-500">Year: {conf.year}</span>}
                 </div>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
             ...
             {configs.length > 0 && !createMutation.isPending && (
               <button type="button" onClick={() => setIsCreating(false)} className="w-full text-sm text-gray-500 hover:underline mt-2">
                 Back to list
               </button>
             )}
          </form>
