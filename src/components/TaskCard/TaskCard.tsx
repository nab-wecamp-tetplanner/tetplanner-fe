...
const TaskCard: React.FC<TaskCardProps> = ({ task, onDeleteTask, onClick, isDissolving, categories, members }) => {
    ...
    const progressPercent = subtaskTotal > 0
        ? Math.round((subtaskDone / subtaskTotal) * 100)
        : task.status === 'completed' ? 100 : task.status === 'in_progress' ? 50 : 0;
    const progressText = subtaskTotal > 0 ? `${subtaskDone}/${subtaskTotal}` : `${progressPercent}%`;
    ...
    return (
        ...
        >
            ...
            <div className="tet-card__body">
                ...
                <h3 className="tet-card__title" onClick={handleCardClick}>{task.title}</h3>

                {/* ── Priority indicator for urgent/high ── */}
                {isHighPriority && (
                    <div className="tet-card__priority-bar">
                        <Flame size={12} />
                        <span>{task.priority === 'urgent' ? 'Urgent Priority' : 'High Priority'}</span>
                    </div>
                )}
                ...
                <div className="tet-card__chips">
                    {task.deadline && (
                        <span className={`tet-card__chip ${task.is_overdue ? 'tet-card__chip--overdue' : ''}`}>
                ...
                <div className="tet-card__footer">
                    {assignedMember ? (
                        <div className="tet-card__assignee" title={assignedMember.name}>
                            ...
                            />
                            <span className="tet-card__assignee-name">{assignedMember.name}</span>
                        </div>
                    ) : (
                    ...
                    <div className="tet-card__footer-actions">
                        ...
                        {subtaskTotal > 0 && (
                            <button className="tet-card__expand-btn" onClick={handleExpandToggle} title="Show subtasks">
                ...
                {expanded && subtaskTotal > 0 && (
                    <div className="tet-card__expand">
                        <div className="tet-card__expand-header">
                            <span>Subtasks</span>
                            <span className="tet-card__expand-count">{subtaskDone}/{subtaskTotal}</span>
                        </div>
                        <ul className="tet-card__subtask-list">
                            {subtaskEntries.map(([title, done]) => (
                                <li key={title} className={`tet-card__subtask ${done ? 'tet-card__subtask--done' : ''}`}>
