  ...
  <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100">
    ...
    <nav className="flex items-center gap-8 text-sm font-medium text-gray-500">
      {navItems.map((item, idx) => (
        ...
        to={item.href}
        className={({ isActive }) => 
          isActive ? "bg-blue-50 text-blue-600 p-2 rounded-md" : "text-gray-500 p-2 rounded-md hover:text-gray-900"
        }
          >
          {item.name}
        </NavLink>
    ...
    <div className="flex items-center gap-4 text-gray-500">
      ...
      <img src="/" alt="User" className="w-8 h-8 rounded-full border border-gray-200" />
    </div>
  </header>
  );
}